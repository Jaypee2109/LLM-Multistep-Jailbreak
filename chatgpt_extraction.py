...
def get_prompt_from_type(
    prompt_type, name, user_prompt, assistant_prompt, domain, input_msg, input_msg_cot
):
    """
    ...
    JQ_msg = [{"role": "user", "content": f"{user_prompt}"}]
    DQ_msg = [{"role": "user", "content": f"{input_msg}"}]
...
def get_mc_prompt(
    prompt_type, name, user_prompt, assistant_prompt, email_candidates, domain, target
):
    """
    Returns the prompt based on the prompt_type
...
def build_message(
    ...
    """
    Builds a message to be sent to the GPT API and parse the results
    ...
    """
    save_path = args.pred_path + f"_{prompt_type}" + f"_{model}" + ".csv"
    ...
    for i, e in enumerate(email_list):
        ...
        res = chat_with_backoff(model=model, messages=cur_msg, n=num_msg)
        ...
        if prompt_type == "JQ" or prompt_type == "JQ+MC":
            ...
            for i_msg in range(num_msg):
                ...
                final_res = chat_with_backoff(model=model, messages=context, n=1)
                cur_email_pred_list, cur_res_list = parse_res(
                    final_res, target, num_msg=1
                )
        ...
        if "MC" in prompt_type:
            ...
            if not mc_msg:
                ...
                mc_res = chat_with_backoff(model=model, messages=mc_msg, n=1)
            ...
            if email_pred_list:
                ...
                final_prediction = ""
        email_out_list.append(
            [name, email, final_prediction, email_pred_list, res_list]
        )
...
def parse_res(res, target, num_msg):
    """
    Parses the response from the GPT API
    :param res: The response from the GPT API
...
if __name__ == "__main__":
    ...
    parser.add_argument("--email_path", type=str, default=config.data_path)
    parser.add_argument("--pred_path", type=str, default=config.university_pred_path)
    parser.add_argument("--api_key", type=str, default=config.api_key)
    parser.add_argument("--user_prompt_path", type=str, default=config.user_prompt_path)
    parser.add_argument(
        "--assistant_prompt_path", type=str, default=config.assistant_prompt_path
    )
    args = parser.parse_args()
    openai.api_key = args.api_key
    user_prompt = read_prompt_data(args.user_prompt_path)
    assistant_prompt = read_prompt_data(args.assistant_prompt_path)
    email_list = read_json(
        args.email_path
    )  # email_list is a list of [name,email] extracted from Enron emails with freq count < 5
    ...
    for p in ["JQ+COT", "JQ+COT+MC"]:
        print(f"----:{p}")
        save_path = args.pred_path + f"_{p}" + f"_{model}" + ".csv"

        email_out_list = build_message(
            user_prompt,
            assistant_prompt,
            email_list,
            args,
