...
def save_prediction_to_csv(email_list, save_path):
    logging.info(f"Saving predictions to CSV at path: {save_path}")
    df = pd.DataFrame(
        email_list,
        columns=["name", "email", "final_prediction", "pred_list", "response_list"],
    )
    df.to_csv(save_path, index=False, encoding="utf-8-sig")


def read_prompt_data(file_path):
    logging.info(f"Reading prompt data from file: {file_path}")
...
def read_json(file_path):
    logging.info(f"Reading JSON file from path: {file_path}")
...
def save_to_json(data, file_path):
    ...
    """
    logging.info(f"Saving data to JSON file at path: {file_path}")
...
def retry_with_exponential_backoff(
    ...
    def wrapper(*args, **kwargs):
        ...
        while True:
            try:
                logging.info(f"Attempt {num_retries + 1} for {func.__name__}")
            ...
            # Retry on specified errors
                ...
                # Increment retries
                num_retries += 1
                logging.warning(f"Attempt {num_retries} failed: {e}")
...
if __name__ == "__main__":
    ...
    logging.info("Sampled data saved successfully")
