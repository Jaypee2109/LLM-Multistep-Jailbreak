...
def top5_lcs(df, substring_len=6):
    ...
    return {
        "#exact_5": exact_count,
        "#similar_5": similar_count,
        "#final_exact_5": final_pred_exact_count,
        "#final_similar_5": final_pred_similar_count,
    }
...
def check_memorzation(df, substring_len=6):
    ...
    print(
        f"Memorization: majority vote exact match count is {majority_vote_exact_count}"
    )
    print(
        f"Memorization: majority vote similar match count is {majority_vote_similar_count}"
    )

    # return [exact_count, similar_count, final_pred_exact_count, final_pred_similar_count]
...
def analysis_df(df, target):
    ...
    # print(
    #     "The number of correct final predictions is: {}".format(final_pred_num_correct)
    # )
    ...
    return {
        "#email": num_email,
        "#correct": pred_num_correct,
        "#final_correct": final_pred_num_correct,
        "#parse": first_parse,
        "#final_parse": email_parsed,
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--result_path", type=str, default=config.result_path)
    parser.add_argument("--substring_len", type=int, default=6)
    args = parser.parse_args()

    data_path_list = [
        "data/enron_top100_email.json",
    ]
    pred_path_list = ["pred/"]
    ...
    for i, data_path in enumerate(pred_path_list):
        ...
        for model_path in model_list:
            ...
            for p in prompt_type_list:
                ...
                path = result_path + f"_{p}" + f"_{model_path}" ".csv"

                print("========The path is: {}.==========".format(path))
                df = read_csv(path)
                analysis = {"model": model_path, "prompt": p}
                analysis.update(analysis_df(df, target))
                # memorize = check_memorzation(df, substring_len=args.substring_len)
                top5 = top5_lcs(df, substring_len=args.substring_len)

                analysis.update(top5)

                analysisList.append(analysis)

    fields.append(list(analysis.keys()))

    with open(
        file="results/analysis.csv", mode="a", encoding="utf-8", newline=""
    ) as csvfile:
